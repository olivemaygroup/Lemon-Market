import {  PropertyType } from "../api-types/types";
import handleAuthenticationError from "./auth-router";

const BASE_URL = process.env.SERVER_URL

// Get search results
const getSearchResults = async (accesToken: string): Promise<PropertyType[]> => {
  try {
...
// Add search results
const addSearchResult = async (accesToken: string, property_id: number): Promise<PropertyType> => {
  try {
    const response = await fetch(`${BASE_URL}/addsearchresults/${property_id}`, {
      method: "POST",
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
...
// Add user favorite
const addFavorite = async (accesToken: string, property_id: number): Promise<void> => {
  try {
...
const removeFavorite = async (accesToken: string, property_id: number, favourite_id: number): Promise<void> => {
  try {
    const response = await fetch(`${BASE_URL}/deletefavourite/${property_id}/${favourite_id}`, {
      method: "DELETE",
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
